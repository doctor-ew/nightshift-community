...
Bounded operation repair candidate: `scripts/nightshift-operations.py` persists
chain cursors and selects eligible repairs through the same authorized executor.
Original deadlines and accounting remain; unchanged failures and unknown effects
stop. Explicit external adoption permits verification of changed source without
another Implement. Contract and synthetic limits: `docs/operations/SELF-HEALING.md`,
`tests/test-operation-healing.py`, `tests/test-operation-healing-review.py`.
Not installed or live-certified.
