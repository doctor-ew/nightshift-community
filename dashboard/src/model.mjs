...
export function ticketProgress(rows, ticket) {
  ...
  const status = ticket.running ? 'Running' : ticket.review && !ticket.review.approved ? 'Awaiting spec approval' : blocker && complete ? 'Conflicting outcomes' : blocker ? 'Blocked' : complete ? 'Complete' : ticket.finished ? 'Run ended · outcome unconfirmed' : ticket.owned === false ? 'Ready to start' : 'Ready to resume';
