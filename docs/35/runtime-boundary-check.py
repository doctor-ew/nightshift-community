...
ROUTING='/private/tmp/nightshift-35-codex-continuation/nightshift-routing.json'
with tempfile.TemporaryDirectory(prefix='nightshift-35-boundary-') as tmp:
    ...
    env=dict(os.environ,PATH=str(binary)+os.pathsep+os.environ['PATH'],NIGHTSHIFT_ROUTING_FILE=ROUTING,NIGHTSHIFT_PROJECT_DIR=str(project),PYTHONDONTWRITEBYTECODE='1',NIGHTSHIFT_TELEMETRY_DIR='off')
    ...
    for provider in ['codex','claude']:
        ...
        if provider=='claude':args+=['--adversarial','--author-provider','codex']
        run(args,callenv)
