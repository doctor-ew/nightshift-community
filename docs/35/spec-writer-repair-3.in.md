# Required scope repair, final spec attempt 3
...
The current docs/35/SPEC.md is NOT approved. Its scope reduction is rejected. Replace its draft, preserving actual claude author identity. Keep spec CONCISE: maximum 220 markdown lines, ideally 150; no conversation history; source entries only exact path:line-range with branch nightshift/35 commit 92b5d71. Scenario document 8 required cases mapping exactly all 8 upstream ACs, plus safety case only if needed. Keep reviews null. Read docs/BEHAVIOR-PROOF.md for exact schema. Use AD technical documentation style.
