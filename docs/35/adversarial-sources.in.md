Read-only independent verification of Codex-authored SPEC. No role/factory dispatch; no writes. Verify each repo Sources entry against actual cited lines and HEAD. Return one complete claim per source with file,line,inspected_files; mark CONFLICT for unsupported/drifted statements. Also independently check upstream acceptance coverage and the controller-added Guardrails against Solution Design; include any substantive inconsistency as a separate CONFLICT claim. Read docs/35/SPEC.md and docs/35/ticket.json; keep searches narrow.
...
Normalize token categories as `fresh_input`, `cache_read_input`, `cache_write_input`, and `output`; retain `reasoning_output` as an output subcategory. For Claude, total is the sum of the four non-overlapping categories. For Codex, validate cache-read plus cache-write does not exceed provider `input_tokens`, set fresh input to the difference, and set total to provider input plus output; cached and reasoning values are never added again. Missing or inconsistent fields increment category-missing/conflict counters instead of producing zero.
...
All upstream ACs apply. Preserve subscription auth, independent gates, existing CLI compatibility, private allowlisted receipts, and non-fatal accounting. Implement the identity, provider normalization, overlap, persistence, and report semantics in Solution Design.
