FINAL BOUNDED REPAIR, COMPONENT PROPOSAL. Your assigned scope is: scripts/nightshift-run-metrics.py ONLY. Implement the full ingest and ticket-report contract with typed allowlisting, immutable storage/replay, dedup/conflicts, overlap, completeness/breakdowns and existing schema1 compatibility. Other workers implement the parser and shell integration.
