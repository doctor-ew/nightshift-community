...
If `{story_key}` is not empty and `/Users/doctorew/shuttlebay/nightshift-community/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/doctorew/shuttlebay/nightshift-community/_bmad/render/bmad-build/nightshift-community-5feee9ac0e93/83de7adfe3ed43292967/sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'full'` and `status: 'draft'`. Go back to `/Users/doctorew/shuttlebay/nightshift-community/_bmad/render/bmad-build/nightshift-community-5feee9ac0e93/83de7adfe3ed43292967/step-02-plan.md` step 6.
...
- **defer** — Everything else: old bugs not caused by this change, ideas for later, groups where every member is `maybe-false` and would be `medium` or `high` if true (record that severity marked unverified, and what would prove it; if it would only be `low`, reject it), or fixes that would edit CLAUDE.md, AGENTS.md, rules, or specs. Add one entry to `/Users/doctorew/shuttlebay/nightshift-community/_bmad-output/implementation-artifacts/deferred-work.md`:
...
If `{story_key}` is not empty and `/Users/doctorew/shuttlebay/nightshift-community/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/doctorew/shuttlebay/nightshift-community/_bmad/render/bmad-build/nightshift-community-5feee9ac0e93/83de7adfe3ed43292967/sync-sprint-status.md` with `{target_status}` = `review`.
